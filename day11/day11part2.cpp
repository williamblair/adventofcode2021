...
/* Returns true if all octopus flashed
 * (energy vals after this iter are all zero) */
static bool runIter( EnergyTable& table )
{
...
static bool testAlg()
{
    ...
    uint64_t flashIter = 0;
    while ( !runIter( energyLevels ) )
    {
        ++flashIter;
        if ( flashIter == 194 || flashIter == 195 || flashIter == 196 )
        {
            std::cout << "Flash iter: " << flashIter << std::endl;
            printEnergyTable( energyLevels );
        }
    }
    ++flashIter;
    if ( flashIter != 195 ) {
        std::cout << "Flash iter failed: " << flashIter << std::endl;
        return false;
    }
