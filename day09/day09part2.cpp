...
struct MapPoint
{
  ...
  void operator=( const MapPoint& other )
  {
    row = other.row;
    col = other.col;
    val = other.val;
...
static void fillBasinPoints(
  HeightMap& map,
  MapPoint& lowPoint,
  std::vector<MapPoint>& res
)
{
  std::queue<MapPoint> pointsToCheck;
  pointsToCheck.emplace( lowPoint );
  res.clear();
  res.push_back( lowPoint );

  std::vector<MapPoint> checkedPoints;

  const uint64_t numRows = map.size();
  const uint64_t numCols = map[0].size();
  
  while ( !pointsToCheck.empty() )
  {
    MapPoint mp = pointsToCheck.front();
    /* Remove from queue */
    pointsToCheck.pop();
    /* Add to already checked list */
    checkedPoints.push_back( mp );
         ...
         map[checkRow][checkCol] != 9 )
    {
      ...
      /* If not already in results, add to results */
      if ( std::find( res.begin(), res.end(), newPoint ) == res.end() )
      {
        res.push_back( newPoint );
      }
      
      /* If not already checked, add to queue to check */
      if ( std::find( checkedPoints.begin(), checkedPoints.end(), newPoint ) ==
           checkedPoints.end() )
      {
        pointsToCheck.emplace( newPoint );
      }
    }
...
static bool testAlg()
{
  ...
  std::vector<std::vector<MapPoint>> basins;
  findLowPoints( testMap, lowPoints );
  for ( MapPoint& mp : lowPoints )
  {
    std::vector<MapPoint> basinRes;
    fillBasinPoints( testMap, mp, basinRes );
    basins.push_back( basinRes );
  }
  /* Sort in order of size from greatest to least */
  std::sort( basins.begin(), basins.end(),
    [](std::vector<MapPoint>& basinA, std::vector<MapPoint>& basinB) {
      return basinA.size() > basinB.size();
    }
  );
  assert( basins.size() >= 3 );
  uint64_t sizesProduct = basins[0].size() * basins[1].size() * basins[2].size();
  if ( sizesProduct != 1134 ) {
    std::cout << "Test failed: " << basins[0].size() << ", "
              << basins[1].size() << ", " << basins[2].size() << std::endl;
    std::cout << "  " << sizesProduct << std::endl;
    return false;
  }
...
int main()
{
  ...
  {
    std::vector<MapPoint> basinRes;
    fillBasinPoints( input, mp, basinRes );
    basins.push_back( basinRes );
  }
  /* Sort in order of size from greatest to least */
  std::sort( basins.begin(), basins.end(),
