...
static uint64_t countNumMatchingUniqueOutputSegments(
    std::vector<std::vector<std::string>>& digOutputs)
{
    std::vector<int> uniqueNumSegments = {
        digitSegmentsMap[1],
        digitSegmentsMap[4],
        digitSegmentsMap[7],
        digitSegmentsMap[8]
    ...
    uint64_t count = 0;
    for ( std::vector<std::string>& strvec : digOutputs )
    {
        for ( std::string& digPattern : strvec )
        {
            if ( std::find(
                    uniqueNumSegments.begin(),
                    uniqueNumSegments.end(),
                    (int)digPattern.size()) !=
                uniqueNumSegments.end() )
            {
                ++count;
...
static bool testAlg()
{
    ...
    uint64_t uniqueCount = countNumMatchingUniqueOutputSegments(
        testInputDigValues
    );
    if ( uniqueCount != 26 ) {
        std::cout << "Test failed: " << uniqueCount << std::endl;
        return false;
    }
    return true;
}
...
int main()
{
    ...
    uint64_t uniqueCount = countNumMatchingUniqueOutputSegments( inputResults );
